...
get_by_test() ->
    ...
    ?assertEqual({[#{<<"name">> => 0}], [ "$['items'][0]"]},
        ejsondb_crud:get_by("$.items", <<"name">>, 0, #{<<"items">> => [#{<<"name">> => 0}]}, #{}, [])),
    ?assertEqual({[#{<<"name">> => <<"oid">>}], [ "$['items'][0]"]},
        ejsondb_crud:get_by("$.items", <<"name">>, <<"oid">>, #{<<"items">> => [#{<<"name">> => <<"oid">>}]}, #{}, [])),
    ?assertError(badarg,
        ejsondb_crud:get_by("$.items", #{}, <<"oid">>, #{<<"items">> => [#{<<"name">> => <<"oid">>}]}, #{}, [])),
    ok.

create_test() ->
    % path doesn't exist
    ?assertEqual({error, not_found}, ejsondb_crud:create("$.inner", <<"id">>, 0, #{<<"value">> => 10}, #{}, '$tail', #{}, [])),
    ?assertEqual({error, not_found}, ejsondb_crud:create("$.inner", <<"id">>, 0, #{<<"value">> => 10}, #{<<"inner">> => 10}, '$tail', #{}, [])),

    % create or error already_exist
    JsonWithInner = #{ <<"inner">> => []},
    Json1 = ejsondb_crud:create("$.inner", <<"id">>, 0, #{<<"value">> => 10}, JsonWithInner, '$tail', #{}, []),
    ...
    ?assertEqual({error, already_exist}, ejsondb_crud:create("$.inner", <<"id">>, 0, #{<<"value">> => 10}, Json1, '$tail', #{}, [])),
...
put_test() ->
    ...
    ?assertEqual({error, not_found},
        ejsondb_crud:put("$.inner", <<"id">>, 0, #{<<"value">> => 10}, #{}, '$tail', #{}, [])),
    ?assertEqual({error, not_found},
        ejsondb_crud:put("$.inner", <<"id">>, 0, #{<<"value">> => 10}, #{<<"inner">> => 10}, '$tail', #{}, [])),

    Json = #{<<"inner">> => []},
    Json1 = ejsondb_crud:put("$.inner", <<"id">>, 0, #{<<"value">> => 10}, Json, '$tail', #{}, []),
    Json2 = ejsondb_crud:put("$.inner", <<"id">>, 0, #{<<"other">> => 10}, Json1, '$tail', #{}, []),
    ?assertEqual(
        #{<<"inner">> => [#{<<"id">> => 0, <<"other">> => 10}]},
        Json2
    ),
...
upsert_test() ->
    % path doesn't exist
    ?assertEqual({error, not_found},
        ejsondb_crud:upsert("$.inner", <<"id">>, 0, #{<<"value">> => 10}, #{}, '$tail', #{}, [])),
    ?assertEqual({error, not_found},
        ejsondb_crud:upsert("$.inner", <<"id">>, 0, #{<<"value">> => 10}, #{<<"inner">> => 10}, '$tail', #{}, [])),

    Json = #{<<"inner">> => []},
    Json1 = ejsondb_crud:upsert("$.inner", <<"id">>, 0, #{<<"value">> => 10}, Json, '$tail', #{}, []),
    Json2 = ejsondb_crud:upsert("$.inner", <<"id">>, 0, #{<<"value">> => 100, <<"other">> => 10}, Json1, '$tail', #{}, []),
    ?assertEqual(
    ...
    Json3 = #{<<"inner">> => [#{<<"id">> => 10}]},
    Json4 = ejsondb_crud:upsert("$.inner", <<"id">>, 0, #{<<"value">> => 10}, Json3, '$tail', #{}, []),
    Json5 = ejsondb_crud:upsert("$.inner", <<"id">>, 0, #{<<"value">> => 100, <<"other">> => 10}, Json4, '$tail', #{}, []),
    ?assertEqual(
...
update_list_i_test() ->
    ...
    ?assertEqual(#{<<"inner">> => [y, y, y]},
        ejsondb_crud:update_list_i("$.inner", #{<<"inner">> => [x, x, x]}, [y, y, y], #{}, [])),
    ok. 
